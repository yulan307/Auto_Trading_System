...
| 接口 | 输入 | 输出 | 说明 |
| --- | --- | --- | --- |
| `build_buy_sub_ml_dataset(...)` | tickers、结束日期、标签窗口 | `DataFrame` | 生成训练数据集 |
| `run_buy_sub_ml_experiment(...)` | tickers、结束日期、版本、训练配置 | 结果字典 | 训练并落盘实验产物 |
| `infer_buy_strength_pct(...)` | tickers、结束日期、模型版本 | CSV 路径字符串 | 用已发布模型做离线推理 |
...
- 数据集构建链路是：`strength_pct 标签 -> 补齐 feature.db -> 选取 hist_* 特征 -> merge -> dropna`。
...
- 推理结果输出是 CSV，而不是数据库写回。

**使用边界**

- `infer_buy_strength_pct` 若找不到模型目录、没有可用特征、或缺少产物文件会直接报错。
- 推理时会自动补齐缺失特征列为 `NA`，然后对必需特征列做 `dropna`。
