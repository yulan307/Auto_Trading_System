...
| `scripts/train_buy_sub_ml.py` | 交互式或非交互式训练与发布买入强度模型 | 新实验产物目录与已发布模型目录 |
| `scripts/infer_buy_sub_ml.py` | 交互式或非交互式选择模型并按 ticker 推理 | 每个 ticker 一个推理 CSV |
...
| `scripts/train_buy_sub_ml.py` | 可用 | `ml.buy_sub_ml` `models/buy` | 交互式训练，或通过 `--mode --model [--output-model]` 走非交互发布 |
| `scripts/infer_buy_sub_ml.py` | 可用 | `ml.buy_sub_ml` `models/buy` | 交互式选模，或通过 `--mode --model` 直接推理 |
