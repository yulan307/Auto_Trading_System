...
- `scripts/train_buy_sub_ml.py`
  ...
  - 运行时菜单：
    ...
    - `2. 更新已有模型`
  - 非交互模式：
    - 传 `--mode new --model <新版本名>` 时直接训练并发布该版本
    - 传 `--mode update --model <已有模型>` 时直接基于该模型配置训练并另存为新目录
    - 传 `--mode update --model <已有模型> --output-model <新版本名>` 时可显式指定更新后的目标版本名
  - 新模型会自动生成带时间戳的新版本名
...
- `scripts/infer_buy_sub_ml.py`
  ...
  - 运行时先列出当前模型供选择
  - 非交互模式：
    - 传 `--mode infer --model <模型名>` 时直接使用指定模型
